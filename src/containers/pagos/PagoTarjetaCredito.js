...
function PagoTarjetaCredito() 
{
  ...
  const [isRegistrarTransaccion, setRegistrarTransaccion] = useState(false);
  ...
  {
    setRegistrarTransaccion(true);
  }


  return (
    isRegistrarTransaccion ?
      <Redirect key="idPago" to=
      {{
          pathname: "/transaccion",
          state: { numeroTarjeta, mesCaducidad, añoCaducidad, titularTarjeta, numeroCVV }
      }}
      />
    :
      <div>
        ...
        <div className="container d-flex flex-column pt-4 bgg-info">
          ...
          <FooterPagos paginaSiguiente={"/"} paginaAnterior={"/metodos_pago"} registrarPedido={registrarPedido}/>
        </div>
